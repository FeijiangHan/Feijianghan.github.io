...
import {
    LineMaterial
} from './jsm/lines/LineMaterial.js';
import {
    LineGeometry
} from './jsm/lines/LineGeometry.js';
import {
    FBXLoader
...
function auto_fps_experimental(recent_incoming_steam) {
    if (window.glb.panelSettings['auto fps']) {
        ...
        if (window.glb.panelSettings['play fps'] > 144) {
            window.glb.panelSettings['play fps'] = 144;
        }
        if (window.glb.panelSettings['play fps'] < 0.1) {
            window.glb.panelSettings['play fps'] = 0.1;
        }
        if (need_fps_change) {
...
var coreReadFunc = function(auto_next = true) {
    if (transfer_ongoing) {
        ...
        if (!DEBUG) {
            setTimeout(coreReadFunc, req_interval * 1000);
        }
        return;
    }
    ...
    request.onload = () => {
        ...
        let inflat;
        if (!DEBUG) {
            inflat = pako.inflate(toUint8Arr(request.response), {
                to: 'string'
            });
        } else {
...
function change_fps(fps) {
    if (Math.abs(window.glb.play_fps - fps) < 0.5) {
        return;
    }
...
function check_flash_life_cyc(delta_time) {
    ...
    window.glb.flash_Obj = window.glb.flash_Obj.filter(function(s) {
        return s['valid'];
    });
    for (let i = window.glb.flash_Obj.length - 1; i >= 0; i--) {
...
function plot_obj_track(object) {
    ...
    if (object.track_init) {
        ...
        if (object.track_n_frame > 2560) {
            alert("track_n_frame is too large, must < 2560")
        }
...
function set_next_play_frame() {
    ...
    if (window.glb.play_pointer >= window.glb.core_L.length) {
        ...
        if (!window.glb.panelSettings['loop to start'] && window.glb.play_pointer == (window.glb.core_L.length - 1)) {
            return;
        }
    }
    parse_time_step(window.glb.play_pointer)
    window.glb.solid_pointer = window.glb.sp_future;
    window.glb.panelSettings['play pointer'] = window.glb.solid_pointer;
    window.glb.sp_future = window.glb.play_pointer;
...
function force_move_all(pp, test_freeze = false) { // 手动调整进度条时触发
    parse_time_step(pp)
    window.glb.solid_pointer = pp;
    window.glb.panelSettings['play pointer'] = window.glb.solid_pointer;
    window.glb.sp_future = pp;
    console.log('set_next_play_frame sp:' + window.glb.solid_pointer + ' spf:' + window.glb.sp_future)
    for (let i = 0; i < window.glb.core_Obj.length; i++) {
        ...
        if (!test_freeze) {
            ...
            object.prev_ro.x = object.rotation.x;
            object.prev_ro.y = object.rotation.y;
            object.prev_ro.z = object.rotation.z
            object.next_ro.x = object.rotation.x;
            object.next_ro.y = object.rotation.y;
            object.next_ro.z = object.rotation.z
...
function render() {
    ...
    for (let key in window.glb.render_callback) {
        // 
        window.glb.render_callback[key]({
            dT: delta,
            T: window.glb.clock.elapsedTime
        })
    }

    if (window.glb.panelSettings['use orthcam']) {
        window.glb.renderer.render(window.glb.scene, window.glb.camera2);
    } else {
        show_cam_orbit();
...
function sprite_like_txt_update() {
    ...
    for (let i = window.glb.core_Obj.length - 1; i >= 0; i--) {
        if (!window.glb.core_Obj[i].text_object) {
            continue;
        }
...
function show_cam_orbit() {
    if (window.glb.panelSettings['show camera orbit']) {
        ...
        if (cam_x) {
            window.glb.scene.remove(cam_x.mesh);
            window.glb.line_Obj.remove(cam_x);
        }
        let cam_y = find_lineobj_by_id('cam_y');
        if (cam_y) {
            window.glb.scene.remove(cam_y.mesh);
            window.glb.line_Obj.remove(cam_y);
        }
        let cam_z = find_lineobj_by_id('cam_z');
        if (cam_z) {
            window.glb.scene.remove(cam_z.mesh);
            window.glb.line_Obj.remove(cam_z);
        }

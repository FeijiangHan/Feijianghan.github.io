...
function parse_init(buf_str) {
    ...
    for (let i = 0; i < each_line.length; i++) {
        ...
        if (str.search(">>set_style") != -1) {
            if (executed_init_cmd[str]) {
                continue;
            } else {
                executed_init_cmd[str] = true;
            }
            parse_style(str)
        }
...
function parse_env(str) {
    ...
    if (style == "terrain") {
        ...
        let height = 13;
        let Segments = 200;
        let need_remove_old = false;
        if (!init_terrain) {
            init_terrain = true;
            need_remove_old = false;
...
function parse_style(str) {
    ...
    if (style == "terrain") {
        ...
        if (!mod_name) {
            console.log('no creative mod selection!');
            return;
        }
        ...
        window.glb.scene.children.filter(function(x) {
            return (x.type == 'GridHelper')
        }).forEach(function(x) {
            x.visible = true
        })
        ...
        if (bgcolor) {
            window.glb.scene.background = new THREE.Color(bgcolor);
        }
    } else if (style == 'star') {
        ...
        if (fontPath) {
            ...
            loader.load(fontPath, function(json) {
                ...
                if (fontLineHeight) {
                    window.glb.font.data.defLineHeight = fontLineHeight
                }
            });
        }
    } else if (style == "sky") {
...
function parse_advanced_geometry_material(str) {
    ...
    if (map) {
        kargs['map'] = THREE.ImageUtils.loadTexture(map)
    }
    if (bumpMap) {
        kargs['bumpMap'] = THREE.ImageUtils.loadTexture(bumpMap)
    }
    if (bumpScale) {
        kargs['bumpScale'] = bumpScale
    }
    if (specularMap) {
        kargs['specularMap'] = THREE.ImageUtils.loadTexture(specularMap)
    }
    if (specular) {
        kargs['specular'] = new THREE.Color(specular)
    }
...
function parse_line(str) {
    ...
    if (!x_arr || !y_arr || !z_arr) {
        alert('Cannot parse line3d, x/y/z_arr missing:', str)
    }
...
function make_flash(type, src, dst, dur, size, color) {
    if (type == 'lightning') {
        ...
        let src_obj = find_obj_by_id(src);
        if (!src_obj) {
            return
        };
        let dst_obj = find_obj_by_id(dst);
        if (!dst_obj) {
            return
        };
        ...
        if (isNaN(src_obj.position.x) || isNaN(src_obj.position.y)) {
            return
        }
...
function parse_core_obj(str) {
    ...
    ro_order = ro_order.replace('Y', 'T');
    ro_order = ro_order.replace('Z', 'Y');
    ro_order = ro_order.replace('T', 'Z');
    ...
    // check parameters
    if (parsed_obj_info['fade_step'] && parsed_obj_info['fade_step'] <= 0) {
        alert('fade_step must >=1 !')
    }

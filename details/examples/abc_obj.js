...
function getTextBackground(text_object, parsed_obj_info) {
    ...
    if (isFinite(dx)) {
        ...
        const material = new THREE.MeshBasicMaterial({
            color: parsed_obj_info['label_bgcolor'],
...
function mesh_dispose(mesh) {
    if (mesh.material.constructor === Array) {
        for (i = 0; i < mesh.material.length; i++) {
...
function makeClearText_old(object, text, parsed_obj_info) {
    ...
    if (!init) {
        ...
        if (!object.label_offset) {
            object.text_object.position.set(object.generalSize, object.generalSize, -object.generalSize);
        } else {
            object.text_object.position.set(object.label_offset[0], object.label_offset[2], -object.label_offset[1]);
        }
        // (init) renderOrder
    ...
    if (!init) {
        ...
        if (object.text_object.background) {
            object.text_object.add(object.text_object.background)
        }
    }
...
function addCoreObj(my_id, color_str, geometry, material, x, y, z, ro_x, ro_y, ro_z, currentSize, label_marking, label_color, opacity, track_n_frame, parsed_obj_info = null) {
    ...
    object.position.y = y;
    object.position.z = z;
    object.next_pos = new THREE.Vector3(0.0, 0.0, 0.0);
    object.next_pos.copy(object.position);
    object.prev_pos = new THREE.Vector3(0.0, 0.0, 0.0);
    object.prev_pos.copy(object.position);
...
function apply_update(parsed_obj_info) {
    ...
    if (object) {
        ...
        load_and_compare_prev(object, parsed_obj_info, 'label_marking');
        object.label_marking = label_marking
        load_and_compare_prev(object, parsed_obj_info, 'label_color');
        object.label_color = label_color
        load_and_compare_prev(object, parsed_obj_info, 'label_opacity');
        object.label_opacity = parsed_obj_info['label_opacity']

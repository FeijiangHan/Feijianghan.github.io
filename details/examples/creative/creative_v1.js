...
class Creative {
    ...
    constructor() {
        ...
        v1Setting.radius = 10;
        v1Setting.radius2 = 10;
        v1Setting.disThreshold = 5;
        v1Setting.n_points = 20;
...
function generate_random_point_walk(setting) {
    ...
    let z = setting.radius * Math.sin(phi2);
    let x = setting.radius * Math.cos(phi2) * Math.cos(theta);
    let y = setting.radius * Math.cos(phi2) * Math.sin(theta);
...
function creative_v1(settings) {
    ...
    window.glb.render_callback[settings.callbackName] = function creative_v1_callback({
        dT,
        T
    }) {
        // console.log(dT)
        ...
        for (let k = v1_group.children.length - 1; k >= 0; k--) {
            if (v1_group.children[k].name.search("-Line-") != -1) {
                ...
                if (delete_condition) {
                    ...
                    v1_pt[i].connected_by -= 1;
                    v1_pt[j].connected_by -= 1;
        ...
        for (let i = 0; i < settings.n_points; i++) {
            ...
            for (let j = 0; j < settings.NNearest - connected_by; j++) {
                ...
                if (!updated_points[name] && distance_mat[p][q] < settings.disThreshold) {
                    ...
                    if (create) {
                        v1_pt[p].connected_by += 1;
                        v1_pt[q].connected_by += 1;
                    }
                }
            }
        }
